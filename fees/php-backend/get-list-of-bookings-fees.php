<?php
  ...
  function getListOfBookingsFees($get, $usrBeId, $lastBeId, $searchQuery) {
    global $link, $linkBD;
    $listOfAllBookings = [];
    ...
    if (sizeof($searchBy) == 0) {
      $sqlAllBookings = $linkBD->query("SELECT beid FROM bookingarchive WHERE status='booked' and hostbeid='$usrBeId' ORDER BY fulldate DESC");
    } else {
      $searchBeIDList = [];
      if (in_array("places", $searchBy)) {
        ...
        $sqlBookingPlaceFilter = $linkBD->query("SELECT beid FROM bookingarchive WHERE hostbeid='$usrBeId' and plcbeid IN ('".implode("', '", $listOfSearchPlace)."')");
      ...
      if (in_array("dates", $searchBy)) {
        ...
        $sqlBookingDatesFilter = $linkBD->query("SELECT beid FROM bookingarchive WHERE hostbeid='$usrBeId' and (".$searchByDateWhere.")");
      ...
      $sqlAllBookings = $linkBD->query("SELECT beid FROM bookingarchive WHERE status='booked' and hostbeid='$usrBeId' and beid IN ('".implode("', '", $searchBeIDList)."') ORDER BY fulldate DESC");
    }
