<?php
  ...
  if ($f_y."-".$f_m."-".$f_d != $t_y."-".$t_m."-".$t_d) {
    if (new DateTime($f_y."-".$f_m."-".$f_d) < new DateTime($t_y."-".$t_m."-".$t_d)) {
      if (!preg_match("/[^0-9\s+-\/]/", $g_phone) || $g_phone == "" || $g_phone == "-") {
        ...
        $sqlPlc = $link->query("SELECT guestNum FROM places WHERE beid='$beId' LIMIT 1");
        $rowPlc = $sqlPlc->fetch_assoc();
        $sqlBooking = $link->query("SELECT beid FROM booking WHERE plcbeid='$beId' and fromd='$org_f_d' and fromm='$org_f_m' and fromy='$org_f_y' and tod='$org_t_d' and tom='$org_t_m' and toy='$org_t_y' and status='booked'");
        if ($sqlBooking->num_rows > 0) {
          $updBookingBeID = $sqlBooking->fetch_assoc()["beid"];
  ...
  function checkBookingTermAvailabilityOutput($sts, $txt, $type) {
    global $beId, $updBookingBeID, $g_name, $g_email, $g_phone, $g_guest, $g_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $unavailableBecauseOfBookingSchedule, $unavailableBecauseOfTechnicalShutdownSchedule, $numOfUnavailable, $deposit, $fullAmount;
    if ($sts == "available") {
      updateBooking($updBookingBeID, $beId, $g_name, $g_email, $g_phone, $g_guest, $g_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $deposit, $fullAmount, true);
    } elseif ($sts == "unavailable") {
      if ($type == "booking") {
        if ($updBookingBeID != $txt && !in_array($txt, $unavailableBecauseOfBookingSchedule)) {
  ...
  function performMainTask() {
    global $updBookingBeID, $beId, $g_name, $g_email, $g_phone, $g_guest, $g_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $deposit, $fullAmount;
    updateBooking($updBookingBeID, $beId, $g_name, $g_email, $g_phone, $g_guest, $g_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $deposit, $fullAmount, true);
  }
  ...
  function connectBookingCancelHandler() {
    global $connectBookingCancelCheckReady, $connectBCancelFailedTxt, $updBookingBeID, $beId, $g_name, $g_email, $g_phone, $g_guest, $g_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $deposit, $fullAmount;
