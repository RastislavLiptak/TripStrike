<?php
  ...
  $org_f_d = mysqli_real_escape_string($link, $_POST['org_f_d']);
  $org_f_m = mysqli_real_escape_string($link, $_POST['org_f_m']);
  $org_f_y = mysqli_real_escape_string($link, $_POST['org_f_y']);
  $org_t_d = mysqli_real_escape_string($link, $_POST['org_t_d']);
  $org_t_m = mysqli_real_escape_string($link, $_POST['org_t_m']);
  $org_t_y = mysqli_real_escape_string($link, $_POST['org_t_y']);
  ...
  if ($placeVerify == "good") {
    if ($f_y."-".$f_m."-".$f_d != $t_y."-".$t_m."-".$t_d) {
      if (new DateTime($f_y."-".$f_m."-".$f_d) < new DateTime($t_y."-".$t_m."-".$t_d)) {
        ...
        $sqlTechnicalShutdown = $link->query("SELECT beid FROM technicalshutdown WHERE plcbeid='$beId' and fromd='$org_f_d' and fromm='$org_f_m' and fromy='$org_f_y' and tod='$org_t_d' and tom='$org_t_m' and toy='$org_t_y' and status='active'");
        if ($sqlTechnicalShutdown->num_rows > 0) {
          $updTechnicalShutdownBeID = $sqlTechnicalShutdown->fetch_assoc()["beid"];
          checkBookingTermAvailability($beId, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay);
        } else {
          error("Technical shutdown not found in database");
        }
      } else {
  ...
  function checkBookingTermAvailabilityOutput($sts, $txt, $type) {
    global $beId, $updTechnicalShutdownBeID, $ts_category, $ts_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay, $unavailableBecauseOfBookingSchedule, $unavailableBecauseOfTechnicalShutdownSchedule, $numOfUnavailable;
    if ($sts == "available") {
      updateTechnicalShutdown($updTechnicalShutdownBeID, $beId, $ts_category, $ts_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay);
      ...
      if ($type == "booking") {
        ...
        if ($updTechnicalShutdownBeID != $txt && !in_array($txt, $unavailableBecauseOfTechnicalShutdownSchedule)) {
  ...
  function performMainTask() {
    global $updTechnicalShutdownBeID, $beId, $ts_category, $ts_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay;
    updateTechnicalShutdown($updTechnicalShutdownBeID, $beId, $ts_category, $ts_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay);
  }

  function connectTechnicalShutdown($listOfTechnicalShutdownsToConnect, $e_fD, $e_fM, $e_fY, $e_tD, $e_tM, $e_tY, $e_firstDay, $e_lastDay) {
    global $link, $connectTSCancelTotal, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay;
    $f_d = $e_fD;
    $f_m = $e_fM;
    $f_y = $e_fY;
    $firstDay = $e_firstDay;
  ...
  function connectTechnicalShutdownCancelHandler() {
    global $connectTechnicalShutdownCancelCheckReady, $connectTSCancelFailedTxt, $updTechnicalShutdownBeID, $beId, $ts_category, $ts_notes, $f_d, $f_m, $f_y, $firstDay, $t_d, $t_m, $t_y, $lastDay;
