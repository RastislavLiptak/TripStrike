<?php
  function getlistOfGuestBookings($get, $usrBeId, $lastBeId, $searchQuery) {
    global $link, $linkBD;
    $listOfAllBookings = [];
    ...
    $limit = 25;
    if ($searchQuery != "") {
      $searchBy = getGuestListSearchType($searchQuery);
    } else {
      $searchBy = [];
    }
    if (sizeof($searchBy) == 0) {
      $sqlAllBookings = $linkBD->query("SELECT beid, usrbeid FROM bookingarchive WHERE hostbeid='$usrBeId' ORDER BY fulldate DESC");
    } else {
      $searchBeIDList = [];
      if (in_array("places", $searchBy)) {
        ...
        $sqlBookingPlaceFilter = $linkBD->query("SELECT beid FROM bookingarchive WHERE plcbeid IN ('".implode("', '", $listOfSearchPlace)."') and hostbeid='$usrBeId'");
      ...
      if (in_array("guest", $searchBy)) {
        $listOfSearchGuests = [];
        $sqlSearchGuest = $link->query("SELECT beid FROM booking WHERE name LIKE '%$searchQuery%'");
        if ($sqlSearchGuest->num_rows > 0) {
          while($rowSearchGuest = $sqlSearchGuest->fetch_assoc()) {
            array_push($listOfSearchGuests, $rowSearchGuest["beid"]);
          }
        }
        $sqlBookingGuestFilter = $linkBD->query("SELECT beid FROM bookingarchive WHERE beid IN ('".implode("', '", $listOfSearchGuests)."') and hostbeid='$usrBeId'");
        if ($sqlBookingGuestFilter->num_rows > 0) {
          while($rowBookingGuestFilter = $sqlBookingGuestFilter->fetch_assoc()) {
            array_push($searchBeIDList, $rowBookingGuestFilter["beid"]);
          }
        }
      }
      if (in_array("dates", $searchBy)) {
        ...
        $sqlBookingDatesFilter = $linkBD->query("SELECT beid FROM bookingarchive WHERE (".$searchByDateWhere.") and hostbeid='$usrBeId'");
      ...
      $sqlAllBookings = $linkBD->query("SELECT beid, usrbeid FROM bookingarchive WHERE beid IN ('".implode("', '", $searchBeIDList)."') ORDER BY fulldate DESC");
    }
    if ($sqlAllBookings->num_rows > 0) {
      while($rowAllBookings = $sqlAllBookings->fetch_assoc()) {
        $checkBookingBeId = $rowAllBookings["beid"];
        $sqlCheckBooking = $link->query("SELECT * FROM booking WHERE beid='$checkBookingBeId'");
        if ($sqlCheckBooking->num_rows > 0) {
          ...
          for ($b = $start; $b < $stop; $b++) {
            ...
            if ($sqlBookings->num_rows > 0) {
              while($rowBookings = $sqlBookings->fetch_assoc()) {
                ...
                $sqlUserBooking = $link->query("SELECT * FROM booking WHERE beid='$bookingBeId' LIMIT 1");
                if ($sqlUserBooking->num_rows > 0) {
                  $rowUserBooking = $sqlUserBooking->fetch_assoc();
                  $guestName = $rowUserBooking['name'];
                  $guestID = "-";
                } else {
                  $guestName = "-";
                  $guestID = "-";
                }
                $usrBeID = $rowBookings['usrbeid'];
                if ($usrBeID != "-") {
                  $sqlAboutUser = $link->query("SELECT * FROM users WHERE beid='$usrBeID' LIMIT 1");
                  if ($sqlAboutUser->num_rows > 0) {
                    $rowAboutUsr = $sqlAboutUser->fetch_assoc();
                    $guestID = getFrontendId($usrBeID);
                  }
                }
  ...
  function getGuestListSearchType($searchQuery) {
