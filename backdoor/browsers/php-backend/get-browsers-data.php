<?php
  ...
  if ($backDoorCheckSignInSts == "good") {
    ...
    while (!$allTrafficDone) {
      $sqlTrafficLog = $linkBD->query("SELECT * FROM usertrafficlog WHERE $periodFilter and browser NOT IN ('".implode("', '", $listOfValues)."') LIMIT 1");
      if ($sqlTrafficLog->num_rows > 0) {
        array_push($listOfValues, $sqlTrafficLog->fetch_assoc()['browser']);
      } else {
        $allTrafficDone = true;
      }
    }
    $totalNum = 0;
    foreach ($listOfValues as $browser) {
      $verifiedNum = 0;
      $unverifiedNum = 0;
      $sqlBrowser = $linkBD->query("SELECT * FROM usertrafficlog WHERE $periodFilter and browser='$browser'");
      if ($sqlBrowser->num_rows > 0) {
        while($rowCC = $sqlBrowser->fetch_assoc()) {
          if ($rowCC['verified'] == "1") {
            ++$verifiedNum;
          } else {
            ++$unverifiedNum;
