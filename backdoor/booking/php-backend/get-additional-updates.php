<?php
  function getAdditionalUpdates($get, $bookingBeId, $lastBeId) {
    global $link, $linkBD;
    $listOfAllUpdates = [];
    $listOfOutputUpdates = [];
    $loadAmount = [];
    $limit = 25;
    $sqlAllUpdates = $linkBD->query("SELECT beid FROM bookingadditionalupdatesarchive WHERE bookingbeid='$bookingBeId' ORDER BY fulldate DESC");
      ...
      if ($stopSts) {
        ...
        if ($get == "list") {
          for ($u = $start; $u < $stop; $u++) {
            ...
            $sqlUpdates = $linkBD->query("SELECT * FROM bookingadditionalupdatesarchive WHERE beid='$updateBeId'");
